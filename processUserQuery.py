...
API_KEY = 'AIzaSyCEz_qcb4VkEAMlFP_fu8OIpdnx73_WCoY'
...
if not (os.path.exists('embeddings.joblib')):
    ...
    print('Successfully created Vector space ')

if(os.path.exists('embeddings.joblib')):
    df = joblib.load('embeddings.joblib')
    question = input('Ask your Question..')   
    question_embedding = np.array(read_chunks.getEmbeddings(question))
...
response = getResponseLLM(prompt=prompt)
